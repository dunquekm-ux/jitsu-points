...
| 8.9 | **Child task-list sorting** — child-side parity with 8.2: `Name / Points` segmented toggle on `HomeScreen` (shown when >1 mission), sorting **within** the Available→Locked→Completed→Missed state groups (grouping preserved). View-only, not persisted. What's New content updated to announce it (Parent Mode). E2E `e2e/child-sort.spec.ts`. Shipped 2026.06.27.1. | ✅ |

#### Phase 8 — User feedback (bonus/demerit flow) — reported 2026.06.27

Reported issue: *"Parent gives a bonus to child 1 — there is no confirmation that the bonus was given. When the parent goes into the child's profile the popup is inconsistent (sometimes shows, sometimes not). The child has no audit log of bonuses received and why. Should the parent also have the audit?"*

Code review of the current flow ([BonusComposer.tsx](app/src/features/parent/BonusComposer.tsx), [DemeritComposer.tsx](app/src/features/parent/DemeritComposer.tsx), [appStore.ts](app/src/core/store/appStore.ts), [HomeScreen.tsx](app/src/features/tasks/HomeScreen.tsx)) confirms all three and pins the root causes:

| # | Task | Status |
|---|---|---|
| 8.10 | **Parent confirmation on bonus/demerit save** — both composers now wrap the save and `navigate('/parent', { state: { toast } })`; `ParentDashboard` shows a transient toast (e.g. "🎉 +15 ⭐ bonus given to Emma") then clears the history state so it doesn't repeat on back/refresh. Save errors reset `saving` so the button can retry. E2E `e2e/bonus-history.spec.ts`. Shipped 2026.06.27.3. | ✅ |
| 8.11 | **Fixed inconsistent child popup (root cause)** — removed in-memory `pendingBonus`/`pendingDemerit`; the overlay is now derived from the **persisted `pointsEvents`** plus a per-child, per-device "seen" set in `localStorage` (`core/ackFeed`: `getUnseenAcks` / `markAckSeen` / `markAllAcksSeen`). `HomeScreen` shows each unseen bonus/demerit exactly once, surviving reloads and reaching other devices. 7-day recency window bounds the one-time upgrade replay; demo-seed (`WelcomeScreen`) and `JoinFamily` call `markAllAcksSeen` so existing history never replays. E2E `e2e/bonus-history.spec.ts`. Shipped 2026.06.27.3. | ✅ |
| 8.12 | **Child-side audit log (points history)** — new `History` tab → "My Points Story" (`features/history/ChildHistoryScreen.tsx`), a kid-friendly newest-first list of every task/bonus/reward/check-in with amount, reason, and date. Powered by shared `features/history/PointsHistoryList.tsx`. Shipped 2026.06.27.3. | ✅ |
| 8.13 | **Parent-side audit log** — tap a child on the dashboard → `features/parent/ChildAuditScreen.tsx` (`/parent/child/:childId`): full per-child ledger + summary (points/level/bonus/check-in counts) + quick Give-Bonus / Check-in actions (reuses `PointsHistoryList`). `DemeritComposer` gained nav-state preselect for parity. (Undo-last-bonus deferred as a future enhancement.) Shipped 2026.06.27.3. | ✅ |
