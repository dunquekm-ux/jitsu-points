...
const pastInstances: TaskInstance[] = pastDays.flatMap(daysBack => {
  ...
  return [
    createTaskInstance(brushTeethTemplate.id, brushTeethMorning.id, emma.id, date, { state: 'completed', completedAt: new Date().toISOString() }),
    createTaskInstance(brushTeethTemplate.id, brushTeethEvening.id, emma.id, date, { state: 'completed', completedAt: new Date().toISOString() }),
    createTaskInstance(makesBedTemplate.id,   makesBedSchedule.id,  emma.id, date, { state: 'completed', completedAt: new Date().toISOString() }),
    createTaskInstance(feedPetTemplate.id,    feedPetMorning.id,    liam.id, date, { state: 'completed', completedAt: new Date().toISOString() }),
    createTaskInstance(feedPetTemplate.id,    feedPetEvening.id,    liam.id, date, { state: 'completed', completedAt: new Date().toISOString() }),
  ];
});

// Today's instances — leave as 'available' so they show up in the home screen
const todayInstances: TaskInstance[] = [
  createTaskInstance(brushTeethTemplate.id, brushTeethMorning.id, emma.id, today, { state: 'available' }),
  createTaskInstance(brushTeethTemplate.id, brushTeethEvening.id, emma.id, today, { state: 'locked' }),
  createTaskInstance(makesBedTemplate.id,   makesBedSchedule.id,  emma.id, today, { state: 'available' }),
...
const emmaEvents = [
  ...
  ...Array.from({ length: 10 }, () => createPointsEvent(emma.id, 5,  'task', { note: 'Brush Teeth' })),
