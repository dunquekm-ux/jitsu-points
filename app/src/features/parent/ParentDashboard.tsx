...
export default function ParentDashboard() {
  ...
  const { profiles, taskTemplates, taskSchedules, taskInstances, pointsEvents, isLoaded, load } = useAppStore();
  ...
  if (!isLoaded) {
    return <div className={styles.screen}><div className={styles.loading}>🥷</div></div>;
  }
      ...
      <div className={styles.body}>
          ...
          <section className={styles.section}>
            ...
            <div className={styles.kidsList}>
              {profiles.map(profile => {
                const pts = selectChildPoints({ pointsEvents } as Parameters<typeof selectChildPoints>[0], profile.id);
                const streak = calculateStreak(taskInstances, profile.id, today);
                const todayTasks = taskInstances.filter(i => i.childId === profile.id && i.date === today);
        ...
        <section className={styles.section}>
          <div className={styles.sectionHeader}>
            ...
            <button className={styles.addBtn} onClick={() => navigate('/parent/task/new')}>+ New</button>
          </div>
          {templateList.length === 0 ? (
            ...
            <div className={styles.taskList}>
              {templateList.map(t => {
                const schedCount = Object.values(taskSchedules).filter(s => s.taskTemplateId === t.id).length;
                  ...
                  <div key={t.id} className={styles.taskRow}>
                    ...
                    <div className={styles.taskInfo}>
                      ...
                      <span className={styles.taskMeta}>
                        ⭐ {t.points} · {schedCount} schedule{schedCount !== 1 ? 's' : ''} · {assignee?.name ?? '?'}
                      </span>
                    </div>
                    <button
