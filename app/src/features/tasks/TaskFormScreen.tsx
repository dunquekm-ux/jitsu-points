...
export default function TaskFormScreen() {
  ...
  const { profiles, taskTemplates, taskSchedules, createTask, updateTask, deleteTask, isLoaded, load } = useAppStore();
  ...
  function addSlot() {
    setSlots(prev => [...prev, { label: 'Evening', startTime: '18:00', endTime: '20:00', reminderTime: null }]);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const data = { title: title.trim(), icon, points, assignedChildId, allowEarlyCompletion, schedules: slots };
      ...
      <div className={styles.body}>
        ...
        <div className={styles.toggleRow}>
          <div className={styles.toggleInfo}>
            ...
            <span className={styles.toggleHint}>Child can complete before the time window opens</span>
          </div>
          <button
        ...
        <div className={styles.fieldGroup}>
          <div className={styles.slotHeader}>
            ...
            {slots.length < 3 && (
              <button className={styles.addSlotBtn} onClick={addSlot}>+ Add slot</button>
            )}
          </div>
          {slots.map((slot, i) => (
            <div key={i} className={styles.slotCard}>
              <div className={styles.slotRow}>
                ...
                />
                {slots.length > 1 && (
                  <button className={styles.removeSlotBtn} onClick={() => removeSlot(i)}>✕</button>
                )}
              </div>
              <div className={styles.slotRow}>
        ...
        {isEdit && confirmDelete && (
          <div className={styles.deleteConfirm}>
            ...
            <div className={styles.deleteBtns}>
              <ChunkyButton variant="ghost" size="sm" onClick={() => setConfirmDelete(false)}>Cancel</ChunkyButton>
              <ChunkyButton variant="ghost" size="sm" onClick={handleDelete}>Yes, delete</ChunkyButton>
            </div>
          </div>
        )}
      </div>
